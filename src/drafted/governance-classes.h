 ...
 *
 *   // network
 *   CGraviumNetwork lvl, network-type, network-status, network-error, milestone-status*
...
private:
    ...
    // Current OBJECT STATUS (see http://govman.gravium.org/index.php/Documentation_:_Status_Field)
    int nStatusID;
...
// // root node
class CGraviumNetwork : public CGovernanceObject
{
private:
    std::string strName;
    std::string strURL;
...
public:
    CGraviumNetwork(UniValue objIn)
    {
...
// // can be under: GraviumNetwork
// //   -- signature requirements : Key1(User)
// class CGraviumNetworkVariable : public CGovernanceObject
// {
// private:
