...
const char * const BITCOIN_CONF_FILENAME = "gravium.conf";
const char * const BITCOIN_PID_FILENAME = "graviumd.pid";
...
bool LogAcceptCategory(const char* category)
{
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL)
        {
            ...
            // "gravium" is a composite category enabling all Gravium-related debug output
            if(ptrCategory->count(string("gravium"))) {
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\GraviumCore
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\GraviumCore
    // Mac: ~/Library/Application Support/GraviumCore
    // Unix: ~/.graviumcore
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "GraviumCore";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/GraviumCore";
#else
    // Unix
    return pathRet / ".graviumcore";
#endif
#endif
}
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()){
        // Create empty gravium.conf if it does not excist
    ...
    {
        // Don't overwrite existing settings so command line settings override gravium.conf
