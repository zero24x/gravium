...
#define patternVersion "Gravium-([0-9]+\\.)?([0-9]+\\.)?([0-9]+)-"

GraviumUpdate::GraviumUpdate(QObject *parent) : QObject(parent)
{
...
bool GraviumUpdate::newVersionAvailable()
{
    Version maxReleaseVersion = getMaxReleaseVersion();
    return maxReleaseVersion > currentVersion;
}

QList<Version> GraviumUpdate::getVersions()
{
    QNetworkAccessManager manager;
    QNetworkReply *response = manager.get(QNetworkRequest(QUrl(GRV_RELEASES)));
    QEventLoop event;
...
Version GraviumUpdate::getMaxReleaseVersion()
{
