...
*Setup instructions for a Gitian build of Gravium Core using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the Gravium
...
- [Building Gravium Core](#building-gravium-core)
...
Clone the git repositories for Gravium Core and Gitian.
...
in the Gravium Core repository under 'Fetch and build inputs' to install sources which require
...
To build Gravium Core (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md#perform-gitian-builds) in the Gravium Core repository.
...
```bash
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/gravium/.git/
    remote: Counting objects: 57959, done.
    remote: Total 57959 (delta 0), reused 0 (delta 0), pack-reused 57958
    Receiving objects: 100% (57959/57959), 53.76 MiB | 484.00 KiB/s, done.
    Resolving deltas: 100% (41590/41590), done.
    From https://github.com/gravium/gravium
...
For example:
```bash
URL=https://github.com/crowning-/gravium.git
COMMIT=b616fb8ef0d49a919b72b0388b091aaec5849b96
./bin/gbuild --commit gravium=${COMMIT} --url gravium=${URL} ../gravium/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit gravium=${COMMIT} --url gravium=${URL} ../gravium/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit gravium=${COMMIT} --url gravium=${URL} ../gravium/contrib/gitian-descriptors/gitian-osx.yml
```
...
and the gravium git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=precise on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../gravium/contrib/gitian-descriptors/*|sort|uniq )
...
cd /some/root/path/
git clone https://github.com/gravium/gravium-detached-sigs.git

BTCPATH=/some/root/path/gravium.git
SIGPATH=/some/root/path/gravium-detached-sigs.git

./bin/gbuild --url gravium=${BTCPATH},signature=${SIGPATH} ../gravium/contrib/gitian-descriptors/gitian-win-signer.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/gravium-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/gravium-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/gravium-osx-build.assert
```
...
[gravium/gitian.sigs](https://github.com/gravium/gitian.sigs/) repository, or if that's not possible to create a pull
request.
